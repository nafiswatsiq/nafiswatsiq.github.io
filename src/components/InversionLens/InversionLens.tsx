...
const InversionLens = ({ src, className }: InversionLensProps) => {
  ...
  const animate = () => {
    ...
    uniformsRef.current.u_time.value += 0.02;
  ...
  return (
    <div ref={containerRef} className={`inversion-lens ${className} `}>
      <img src={src} style={{ display: "none" }} alt="Inversion Lens" className="w-full h-full object-cover" />
    </div>
  );
}
